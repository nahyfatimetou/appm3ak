...
class _SubmitLocationScreenState extends ConsumerState<SubmitLocationScreen> {
  ...
  final Map<String, Uint8List> _imageBytes = {}; // Cache pour les bytes des images
  ...
  Future<void> _pickImages() async {
    ...
    if (images != null && images.isNotEmpty) {
      // Charger les bytes des images pour l'affichage
      for (var image in images) {
        try {
          final bytes = await image.readAsBytes();
          _imageBytes[image.path] = bytes;
        } catch (e) {
          print('⚠️ Erreur lors du chargement de l\'image: $e');
        }
      }
      setState(() {
                          ...
                          ClipRRect(
                            ...
                            child: FutureBuilder<Uint8List?>(
                              ...
                              future: () async {
                                ...
                                if (_imageBytes.containsKey(image.path)) {
                                  return _imageBytes[image.path];
                                }
                                try {
                                  final bytes = await image.readAsBytes();
                                  if (mounted) {
                                  ...
                                  print('⚠️ Erreur lors de la lecture de l\'image: $e');
                                  return null;
                                }
                              }(),
                            ...
                            child: CircleAvatar(
                              ...
                              child: IconButton(
                                ...
                                onPressed: () {
                                  final image = _selectedImages[index];
                                  setState(() {
                                    _imageBytes.remove(image.path); // Remove from cache
